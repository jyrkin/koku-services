...
public class MessageServiceTest {
    ...
    public void archiveMessage() {
        ...
        assertEquals("Message moved to sender's Archived Outbox folder: ", FolderType.Archive_Outbox, serviceFacade.getMessageById(messageId)
                .getMessageType());
    }
    ...
    public void getMessagesWithPaging() {
        ...
        assertEquals("Oldest message is the last: ", "subject_0", serviceFacade.getMessages(fromUserId, FolderType.Outbox, new MessageQuery(1, 10))
                .get(0).getSubject());
    }
    ...
    public void sendRequestWithTwoQuestions() {
        ...
        final long requestId = serviceFacade.sendRequest(fromUserId, "test request", Collections.singletonList(toUserId),
                "read-only form of request", questions, new ArrayList<MultipleChoiceTO>(), RequestTemplateVisibility.Creator, null, null);
    ...
    public void respondToRequest() {
        ...
        final Long requestId = serviceFacade.sendRequest(fromUserId, "test request", toUsers, "read-only form of request", questions,
                new ArrayList<MultipleChoiceTO>(), RequestTemplateVisibility.Creator, null, null);
    ...
    private MessageSummary getMessageById(final List<MessageSummary> messages, final long messageId) {
        for (final MessageSummary message : messages) {
    ...
    public void testTotals() {
        ...
        final Long oldRequestId = serviceFacade.sendRequest(fromUserId, "test request", Collections.singletonList(toUserId),
                "read-only form of request", questions, new ArrayList<MultipleChoiceTO>(), RequestTemplateVisibility.Creator, oldDate, null);
        assertEquals(oldSentTotal + 1, serviceFacade.getTotalRequests(fromUserId));
    ...
    public void autoarchiveOldMessages() {
        ...
        assertEquals("Message moved to sender's Archived Outbox folder: ", FolderType.Archive_Outbox, serviceFacade.getMessageById(messageId)
                .getMessageType());
        assertEquals("Message is still in the sender's Outbox folder: ", FolderType.Outbox, serviceFacade.getMessageById(messageNewId)
                .getMessageType());
